...
const ChangePasswordAssoc = () => {
    const token = localStorage.getItem('userToken');
    const decodedToken = jwtDecode(token);
    const idAssoc = decodedToken.idAssoc;
    ...
    const handleChangePassword = async (e) => {
        ...
        setLoading(true);
        try {
            await api2.put(`/Associacoes/${user.idAssociacao}/change-password`, { novaSenha: newPassword });
            toast.success('Senha atualizada com sucesso!');
            setLoading(false);
            setTimeout(() => {
                navigate('/Usuarios/perfil/edit/assoc');
            }, 1800);
        } catch (error) {
...
export default ChangePasswordAssoc;